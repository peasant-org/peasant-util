...
public class HttpUtils {
     ...
     *
     * @param urlParam
     * @param params 添加到URL中的参数，形式如:url?params
     * @param is content的内容输入流
     * @param properties 设置请求的属性参数,{@link  HttpURLConnection}
     * @param charset 返回内容的解码字符集
     * @return
     */
    public static String sendPost3(String urlParam, Map<String, Object> params, InputStream is, Map<String, String> properties, String charset) {
        StringBuffer resultBuffer = null;
        // 构建请求参数  
        ...
        // 发送请求  
        try {
            URL url = sbParams.length() > 0 ? new URL(urlParam + "?" + sbParams.substring(0, sbParams.length() - 1)) : new URL(urlParam);
            con = (HttpURLConnection) url.openConnection();
            con.setRequestMethod("POST");
            con.setDoOutput(true);
            con.setDoInput(true);
