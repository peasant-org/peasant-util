...
public abstract class UUIDEntity implements Serializable {
     ...
     * 但使用JPA自动生成表时，若有外键引用引uuid主键，且不允许外键列为null时，则必须在外键字段上 使用columnDefinition =
     * "char(36) not null"指定，否则生成外键字段时默认使用引用的主键字段
...
//        }
    ...
    public String toString() {
        ...
        for (Method m : methods) {
            if (m.getParameterTypes().length == 0 && m.getName().startsWith("get") && !m.getReturnType().equals(Void.class)) {
                try {
                    sb.append('[').append(m.getName().substring(3)).append('=').append(m.invoke(this)).append(']');
